...
function PostForm({ mode }) {
    ...
    const { id } = useParams();// Url 에서 :id같은 동적 파라미터 값을 가져옴. 문자열
    ...
    const uploadMutation = useMutation({
        mutationFn: (file) => uploadImage(file), // 'File' 대신 'file' 사용
        onSuccess: (result) => {
            setImageUrl(result.imageUrl);
            // 업로드 성공 후 로딩 상태를 여기서 명시적으로 끄지 않아도 됩니다.
            // TanStack Query의 isPending/isError/isSuccess 상태로 자동 관리됩니다.
        },
        onError: () => {
            alert('이미지 업로드 실패');
        }
